...
cluster = 2; % 0 for 'totten', 1 for 'andes', 2 for epica
...
if perform(org,['Greenland_ISMIP7Prep_' climate_model '_Historical']),% {{{
	...
	%md.timestepping.time_step_max=0.04;
...
if perform(org,['Greenland_ISMIP7Prep_CESM2-WACCM_SSP585']),% {{{

	md=loadmodel(org,['Greenland_SnapshotInversion']);
	...
	for r = 1:length(regions)
		...
		disp(['	Loading ' region_name ' transient run']);
		tmp = loadmodel(['Models/' region_name '_TransientCalibrationRun_101_103']);
		disp(['	Loading ' region_name ' transient calibrated friction coefficient']);
		md.friction.coefficient(tmp.mesh.extractedvertices) = tmp.results.TransientSolution(1).FrictionCoefficient;
		disp(['    Resetting values for ' region_name ', starting projection at ' num2str(projection_start_time)]);
		step_times = [tmp.results.TransientSolution(:).time];
		step_pos = find(abs(step_times-projection_start_time)==min(abs(step_times-projection_start_time)));
		disp(step_times(step_pos));
		tmp = transientrestart(tmp,step_pos);
		disp(['	Transferring transient restart details from ' region_name ' to Greenland']);
		md.initialization.vx(tmp.mesh.extractedvertices) = tmp.initialization.vx;
		md.initialization.vy(tmp.mesh.extractedvertices) = tmp.initialization.vy;
		md.initialization.vz(tmp.mesh.extractedvertices) = tmp.initialization.vz;
		md.initialization.vel(tmp.mesh.extractedvertices) = tmp.initialization.vel;
		md.initialization.pressure(tmp.mesh.extractedvertices) = tmp.initialization.pressure;
		md.initialization.temperature(tmp.mesh.extractedvertices) = tmp.initialization.temperature;

		md.mask.ice_levelset(tmp.mesh.extractedvertices) = tmp.mask.ice_levelset;
		md.mask.ocean_levelset(tmp.mesh.extractedvertices) = tmp.mask.ocean_levelset;

		md.geometry.base(tmp.mesh.extractedvertices) = tmp.geometry.base;
		md.geometry.thickness(tmp.mesh.extractedvertices) = tmp.geometry.thickness;
		md.geometry.surface(tmp.mesh.extractedvertices) = tmp.geometry.surface;
	end

	disp('   Prescribing SMB');
   md.smb = interpISMIP7GreenlandSMB(md,'CESM2-WACCM','ssp585',[projection_start_time, projection_end_time]);
...
if perform(org,['Greenland_ISMIP7Run_CESM2-WACCM_SSP585_' int2str(projection_start_time) '-' int2str(projection_end_time)]),% {{{
   ...
   md.timestepping.final_time=projection_end_time;
	...
	% Set the friction coefficient back to the snapshot if requested
	if strcmp(forecast_friction, 'Snapshot')
		disp('Setting snapshot friction coefficient')
		tmp = loadmodel('Models/Greenland_SnapshotInversion');
		md.friction.coefficient = tmp.friction.coefficient;
	end

	if calving
		md.transient.ismovingfront=1;
	
		% melting rate at the front is prescribed by ISMIP6 parameterization 
